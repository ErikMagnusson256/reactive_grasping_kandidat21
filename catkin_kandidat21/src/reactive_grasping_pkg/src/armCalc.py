...
'''
Interface between our ROS network and the Universal Robot Driver which 
in turn controls the UR10 Robot armThis interface enables easy functions 
to control the robot for simple grasping sequences
'''
class UR10_robot_arm:

    '''
    TODO not done - is a simplified version that only saves
    position with a fixed preset orientation
    Reads the current robot TCP translation and rotation from the /tf
    topic that is published by the Universal Robots Driver
    ...
    def read_gripper_translation_rotation(self):
        try:
            trans = self.tfBuffer.lookup_transform(self.frame_name,
                                                   'tool0_controller', rospy.Time())
            ...
            #roll pitch yaw
            '''self.Rx,self.Ry, self.Rz = tf_conversions.transformations.euler_from_quaternion(
            ...
            #print('qx:', trans.transform.rotation.x, 'qy:', trans.transform.rotation.y,
            # 'qz:', trans.transform.rotation.z, 'qw:', trans.transform.rotation.w)
    ...
    '''
    Returns true if the robot arm is at the position and pose given as 
    arguments. Arguments includes a tolerance in mm and Rad
    
    Parameters
        x : global position in meters
        y
        z
        Rx : global rotation of TCP in Rad
        Ry
        Rz
        tol_mm : How close to the target it accepts
        tol_rad : How close to the target it accepts
    
    '''
    def is_at_position(self, x, y, z, Rx, Ry, Rz, tol_mm, tol_rad):
        self.read_gripper_translation_rotation()
        if math.isclose(self.xTranslation, x, abs_tol=tol_mm*0.001) and \
                math.isclose(self.yTranslation, y, abs_tol=tol_mm*0.001) and \
                math.isclose(self.zTranslation, z, abs_tol=tol_mm*0.001) and \
                math.isclose(self.Rx, Rx, abs_tol=tol_rad) and \
                math.isclose(self.Ry, Ry, abs_tol=tol_rad) and \
                math.isclose(self.Rz, Rz, abs_tol=tol_rad):
            return True
        else:
            return False
    ...
    Moves the TCP endposition a certain distance to the right in 
    relation the the gripper Does some calculations on where the new TCP 
    position should be and then calls another method to execute the 
    ...
    def move_gripper_R(self, distance_mm):
        ...
        # points head forward
        self.read_gripper_translation_rotation()
        while self.xTranslation == 0 and self.yTranslation == 0 \
                and self.zTranslation == 0:
            self.read_gripper_translation_rotation()
    ...
    Moves the TCP endposition a certain distance to the right in relation 
    the the gripper Does some calculations on where the new TCP position 
    should be and then calls another method to execute the command to the robot
    ...
    def move_gripper_L(self, distance_mm):
        ...
        # points head forward
        self.read_gripper_translation_rotation()
        while self.xTranslation == 0 and self.yTranslation == 0 and \
                self.zTranslation == 0:
            self.read_gripper_translation_rotation()
    ...
    def move_gripper_up(self, distance_mm):
        # move vertically, or move "up" from the grippers perspective?
        # keep gripper at same direction?
    ...
    '''
    Moves the robot TCP to a wanted position with a wanted pose, moves 
    linearly with a predefined low speed
    ...
    '''
    Enabled the use of depth compensation when using the RG2-FT parallell 
    jaw gripper, meaning it will move the TCP backwards when
    closing the gripper to keep the fingers locked in space.
    
    Does this by approximating the gripper jaw motion to a circle path and 
    then calculating how much forwards/backwards it needs to move
    ...
    def depth_compensation_gripper(self, current_width_mm, target_width_mm):
        #Moves the TCP forwards or backwards depending on how gripper
        # is opening/closing

        radious = 170
        current_depth_mm = radious - np.sqrt((radious**2 -
                                              current_width_mm**2))
        target_depth_mm = radious - np.sqrt((radious**2 -
                                             target_width_mm**2))

        self.execute_movel_cmd(self.xTranslation, self.yTranslation +
                               current_depth_mm*0.001 - target_depth_mm*0.001,
                               self.zTranslation, self.Rx, self.Ry, self.Rz)

        return True

    '''
    Executes a movel command to the robot by sending an movel command in '
    urscript wich is then interpret by the robot moves linearly between 
    where it currently is and where it should go
    ...
    def execute_movel_cmd(self,x, y, z, Rx, Ry, Rz, a=0.05, v=0.05,r=0):
        ...
        movel_cmd = 'movel(p[' + str(x) + ',' + str(y) + ',' + str(z)+ ',' + \
                    str(Rx) + ',' + str(Ry) + ',' + str(Rz) + '],' + str(a) + \
                    ',' + str(v) + ',' + str(r) + ')'
    ...
    '''
        Test sequence that moves the robot in a Line in the XY plane with 
        length of 150 mm
        Used to verify functionality of methods created

        WARNING: using timers is not optimal, use the method is_at_position( ... ) 
        to verify that it is in the right position instead
    ...
    '''
    Test sequence that moves the robot in a square in the XZ plane with 
    sides of 200 mm
    Used to verify functionality of methods created
    ...
    def move_square_200_mm(self):
        while not rospy.is_shutdown():
            ...
            print('xtrans:', self.xTranslation)
            # reads current robot poistion (not pose atm)
            self.read_gripper_translation_rotation()
    ...
    def test(self):
        while not rospy.is_shutdown():
            ...
            #print('Albins x:',self.xTranslation, 'y:', self.yTranslation,
            # 'z:', self.zTranslation, 'Rx:', np.rad2deg(self.Rx), 'Ry:',
            # np.rad2deg(self.Ry), 'Rz:', np.rad2deg(self.Rz), ' OBS nu äre i grader')
            print('Data values from /tf: x', self.xTranslation, ' y',
                  self.yTranslation, ' z', self.zTranslation)
    ...
    '''
    Constructor method that is called each time a new instance of this 
    bject is created Verifies that it has read a position before exiting 
    init => all other methods can be used directly after creatign this object
    Parameters
        self
    Returns
        None
    ...
    def __init__(self):
       ...
       # self.test_tf_grunkor()
        ...
        self.script_cmd_publisher = rospy.Publisher(
        ...
        self.gripper_offset = 0 # vector length offset that tells how far from
       # TCP the gripper TCP is at a given moment. Can change depending on
       # how open/close the gripper is
        self.read_gripper_translation_rotation()

        self.script_cmd_publishers = rospy.Publisher(
            '/ur_hardware_interface/script_command', String,
            queue_size=1)
    ...
    '''
    Used for testing functionality, is never called upon and 
    executed when creating instance of this object
    Parameters
        None
    Returns
        None
