...
'''
Test sequence - centers the fingers around an object, closes the gripper 
and starts to grip
...
def stage_test():
    ...
    pub_cmd_maincontroller = rospy.Publisher('/gripper_interface/gripper_cmd/',
...
'''
Method representing the grip sequence, centers the fingers around the object 
and closes the fingers so that a basic grasp is being established
Starts by opening the gripper
Paramters
    tolerance - different objects could need a seperate tolerance in when 
    grasping due to material differance
Returns
    None
Throws
    None
...
'''
Makes sure object doesn't slip, adjusts force and width to ensure not 
dropping it
...
'''
Test method to see how well arm movement can be executed and at the same time 
ensure that the arm is at position
...
def stage_1():
    ...
    while not arm.is_at_position(0.0857, -0.7475 - 0.055, Z+ 0.1, np.pi / 2, 0, 0,
                                 50, 0.5):
        print('not at pos yo')
        slip_check()
...
def stage_2():
    ...
    while not arm.is_at_position(-0.6044, -0.8730, 0.0401, np.pi / 2, 0, 0,
                                 10, 0.5):
        slip_check()
    ...
    while not arm.is_at_position(-0.6044, -0.8730 + 0.1, 0.0401, np.pi / 2, 0, 0,
                                 50, 0.5):
        continue
...
def stage_3():
    ...
    while not arm.is_at_position(0.2724, -0.7581, 0.0120, np.pi / 2, 0, 0,
                                 10, 0.5):
        print('not at pos')
        continue
    print('at tynngd')
    ...
    while not arm.is_at_position(0.2724, -0.7581 - 0.055, 0.0120, np.pi / 2, 0, 0,
                                 10, 0.5):
        continue
...
def stage_5():
    ...
    while not arm.is_at_position(-0.6044, -0.8730, 0.0277 - 0.03, np.pi / 2, 0, 0,
                                 10, 0.5):
        slip_check()
    ...
    time.sleep(1)
    arm.move_gripper_backwards(100)
    while not arm.is_at_position(-0.6044, -0.8730 + 0.1, 0.0277, np.pi / 2, 0, 0,
                                 10, 0.5):
        continue
...
def stage_6():
    ...
    while not arm.is_at_position(0.4472, -0.7139, 0.0638, np.pi / 2, 0, 0,
                                 20, 0.5):
...
def stage_8():
    ...
    while not arm.is_at_position(-0.6044, -0.8730, 0.0638 + 0.03, np.pi / 2, 0, 0,
                                 10, 0.5):
        slip_check()
    ...
    while not arm.is_at_position(-0.6044, -0.8730 + 0.1, 0.0638, np.pi / 2, 0, 0,
                                 10, 0.5):
        continue
...
if __name__ == '__main__':
    ...
    This sequence tries to grip three types of objects at an approximate position 
    and drops them off at a known location
    The approximate position allows our controlling algorithms to make final small 
    adjustments to grasp the object
    '''
    stage_0() #moves to the cup position and positions the fingers approximatly
    # around the cup, gripps the cup
    stage_1() #moves the cup up 100 mm,checks for slip and adjusts
    stage_2() #drops off the cup at dropoff position, checks for slip and
    # adjusts while moving
    stage_3() #moves to the approximate weight position, places the fingers
    # roughly around the object, makes small adjustments and grips the weight
    stage_4() #moves the weight up 100 mm,checks for slip and adjusts
    stage_5() #drops off the weight dropoff position, checks for slip and
    # adjusts while moving
    stage_6() #moves to the approximate weight position, places the fingers
    # roughly around the object, makes small adjustments and grips the weight
